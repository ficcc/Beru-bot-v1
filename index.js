...
const { prefix,discord_owner_id } = require('./config.js');
const db = require('quick.db');
const Canvas = require('canvas');
const Kitsu = require('kitsu');
const nineanime = require('9anime');
...
client.registry
  ...
  .registerGroups([
    ...
    ['anime', ':tv: Anime and Manga Commands:'],
...
client.once('ready', () => {
  ...
  client.user.setActivity(`you smile`, {
    type: 'WATCHING',
...
triggerWords = ["ez","retard","noob"]
triggerReplies = ["Good for you", "?"]

client.on("message", msg => {

    if (msg.mentions.has(client.user.id)) {
        msg.channel.send("Yes!! My Liège.");
    }
    ...
    if (triggerWords.some(word => msg.content.includes(word))){
        const triggerReply = triggerReplies[Math.floor(Math.random()*triggerReplies.length)]
        msg.reply(triggerReply)
    }

});       


client.login(tokenkey);
